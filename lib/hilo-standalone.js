    ...
    tick:function(deltaTime){
        ...
        if(target){
            ...
            if(bounds){
                ...
                scroll.y = Math.min(Math.max(scroll.y, bounds[1]), bounds[0] + bounds[3]);
            }
        }
        else{
            scroll.x = 0;
...
var ParticleSystem = (function(){  
     ...
     * @class Particle 粒子
     * @inner
     * @property {Number} vx x速度
     * @property {Number} vy y速度
     * @property {Number} ax x加速度
     * @property {Number} ay y加速度
     * @property {Number} scaleV 缩放变化速度
     * @property {Number} alphaV 透明度变换速度
     * @property {Number} rotationV 旋转速度
     * @property {Number} life 存活时间
    */
